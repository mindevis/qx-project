...
### 3.1 SignPath Foundation (OSS — in progress)

QXSystem applies to the **[SignPath Foundation](https://signpath.org/)** for free code signing of open-source projects. Windows release builds of `qx-launcher.exe` are intended to be signed through this program in CI (`prod-release.yml`).

Public statement (also on [mc.qx-dev.ru/launcher](https://mc.qx-dev.ru/launcher) and [launcher-download.md](./launcher-download.md)):

> Windows builds of QX Launcher are code-signed through the SignPath Foundation open-source code signing program.

After approval, signing runs in GitHub Actions via `signpath/github-action-submit-signing-request` — private keys stay on SignPath HSM.

### 3.2 Commercial OV / EV certificate

Alternative if SignPath is unavailable:
...
### 3.4 Self-signed certificate (development only)
...
*See also [launcher-download.md](./launcher-download.md), [privacy.md](./privacy.md), [post-mvp.md#launcher-update](./post-mvp.md#launcher-update), [security-legal.md](./security-legal.md) §10.*
